...
def get_todolist():
    ...
    如果不存在 'status' 参数，将返回所有的待办事项（排除已放弃的）。
    ...
    if status is not None:
        ...
        elif status == '2' or status == 'abandoned':
            # 专门查询已放弃的项目
            cursor.execute("SELECT * FROM todolist WHERE status = 2")
        else:
            # 查询指定状态
            cursor.execute("SELECT * FROM todolist WHERE status = %s", (status,))
    else:
        # 默认查询所有非已放弃的项目（状态为0或1）
        cursor.execute("SELECT * FROM todolist WHERE status != 2")
...
def abandon_todolist():
    ...
    这个函数从客户端获取 JSON 数据，提取 todolist_id，然后将对应的待办事项状态设置为2（已放弃），
    并记录放弃时间。

    请求参数（JSON）:
        todolist_id (int): 要放弃的待办事项ID

    返回值：
        str：操作结果的描述

    使用示例：
        POST /todolist/abandon
        {
            "todolist_id": 1
        }
        
        返回: 'Todolist abandoned successfully'
    ...
    query = "UPDATE todolist SET status = 2, do_time = %s WHERE id = %s"
    cursor.execute(query, (abandon_time, todolist_id))
    db.commit()
